...
public class MessageFrame extends JDialog {
	...
	public MessageFrame(final String message, final int time) {
		this.setTitle(LanguageManager.getString("message.title")); // Le titre de la fenêtre.
		...
		final JLabel lblCountdown = new JLabel(LanguageManager.getString("message.footer", time));
		...
		new Timer().scheduleAtFixedRate(new TimerTask() {
			...
			public final void run() {
				lblCountdown.setText(LanguageManager.getString("message.footer", currentTime--));
